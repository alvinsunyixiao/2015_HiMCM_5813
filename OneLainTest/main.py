...
vdesire = 25 #desired speed with infinite gap
w = 2 #minimum stationary distance
T = 2   #refresh timing = (T-1)*refreshRate
time = 7 #time interval between cars
...
class CAR:
    ...
    def __init__(self,v,x,road,plannumber):
        self.velocity = [v]
        self.safespeed = []
        self.position = [x]
        self.road = road
    ...
    def findfrontcar(self):  #return the car object in front of self
        if self in self.road.carsleft:
            ...
            if self == self.road.carsright[0]:
                ...
                virtualCar = CAR(0,virp,self.road,0)
    ...
    def updateZ(self):
        ...
        if self.position[-1]>=self.road.signPos:
            if self.plannumber == 1:
                ...
                if self.position[-1]<=self.road.rightlength-10:
                    self.z = 0
                else:
                    self.z = 1
    ...
    def whetherMerge(self):
        ...
        if previousIndex == -1:
            if len(self.road.carsleft) == 0:
                ...
                frontLeftCar = self.road.carsleft[0]
                t = 0.1
                allowed = True
                while t < mergeTime:
...
class ROAD:
    ...
    def insertAcarleft(self):
        self.carsleft.append(CAR(vdesire,0,self,0))

    def insertAcarright(self):
        self.carsright.append(CAR(vdesire,0,self,universalplannum))



myRoad = ROAD(3500,3000,0)
...
for car in stats:
    print len(car.velocity)
    print sum(car.velocity)/len(car.velocity)
